...
const create_quiz = async (req, res) => {
    try {
        const { title, description, category, timeLimit, questions: questionIds } = req.body;
        ...
        if (questionIds && questionIds.length > 0) {
            ...
            if (foundQuestions.length !== questionIds.length) {
                ...
                return res.status(400).json({
                    status: "failed",
                    message: "Validation failed: One or more provided question IDs do not exist.",
        ...
        res.status(201).json({ status: "success", data: populatedQuiz || new_quiz });

    } catch (error) {
         if (error.name === 'ValidationError') {
             const errors = Object.values(error.errors).map(el => el.message);
             return res.status(400).json({ status: "failed", message: "Database validation failed", errors: errors });
         }
         res.status(500).json({ status: "failed", message: "Failed to create quiz." });
    }
};

const get_all_quizzes = async (req, res) => {
    try {
        const quizzes = await Quiz.find().select('-questions').populate('createdBy', 'username');
        res.status(200).json({ status: "success", data: quizzes });
    } catch (error) {
        res.status(500).json({ status: "failed", message: "Failed to retrieve quizzes." });
    }
};
...
const get_quiz = async (req, res) => {
    try {
        ...
        res.status(500).json({ status: "failed", message: "Failed to retrieve quiz details." });
    }
};
    ...
    try {
        ...
        if (updateData.questions && updateData.questions.length > 0) {
             ...
             if (foundQuestions.length !== updateData.questions.length) {
                const foundIds = foundQuestions.map(q => q._id.toString());
                const invalidIds = updateData.questions.filter(id => !foundIds.includes(id));
                 return res.status(400).json({
                    status: "failed",
                    message: "Validation failed: One or more provided question IDs do not exist.",
                    invalidQuestionIds: invalidIds
        ...
        const updatedQuiz = await Quiz.findByIdAndUpdate(id, updateData, {
            new: true,
            runValidators: true
        }).populate('questions').populate('createdBy', 'username email');
        ...
        res.status(200).json({ status: "success", data: updatedQuiz });
         ...
         res.status(500).json({ status: "failed", message: "Failed to update quiz." });
    }
};
...
const delete_quiz = async (req, res) => {
     try {
        ...
        res.status(200).json({ status: "success", message: "Quiz deleted successfully", data: deletedQuiz });

    } catch (error) {
        res.status(500).json({ status: "failed", message: "Failed to delete quiz." });
    }
};
