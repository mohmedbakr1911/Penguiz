...
const create_question = async (req, res) => {
    try {
        ...
        res.status(201).json({ status: "success", newQuestion });
       ...
       res.status(500).json({ status: "failed", message: "Failed to create question." });
    }
};
...
const get_all_questions = async (req, res) => {
    try {
        ...
        res.status(500).json({ status: "failed", message: "Failed to retrieve questions." });
    }
};
...
const get_question = async (req, res) => {
    try {
        ...
        res.status(500).json({ status: "failed", message: "Failed to retrieve question details." });
    }
};
...
const update_question = async (req, res) => {
    try {
       ...
       res.status(500).json({ status: "failed", message: "Failed to update question." });
    }
};
...
const delete_question = async (req, res) => {
     try {
        ...
        res.status(500).json({ status: "failed", message: "Failed to delete question." });
    }
};
